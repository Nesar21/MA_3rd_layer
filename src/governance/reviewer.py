...
class ReviewerGate:
    ...
    def __init__(self, manifest: Dict[str, Any]):
        ...
        # Rejects 'google.com' (missing scheme) or garbage.
        # Explicitly allowing 'N/A' as sentinel per spec.
    ...
    def evaluate(self, contract: Dict[str, Any]) -> ReviewResult:
        ...
        schema_errors = self._check_schema(contract)
        ...
        # 3. Weak Format Validation (RG-WEAK-FORMAT-005)
        if len(errors) < self.max_errors:
            errors.extend(self._check_weak_formats(contract))

        # 4. DAG Validation (RG-DAG-004)
        if len(errors) < self.max_errors:
            errors.extend(self._check_dag(contract))

        # 5. Compute Locked Sections (Issue #1 Fix)
        # We lock sections that have ZERO errors associated with them.
    ...
    def _compute_locked_sections(self, contract: Dict[str, Any], errors: List[ReviewError]) -> List[str]:
        """
        Determines which top-level keys are safe to lock.
        Locking Rule: A section is locked if NO errors reference it.
        """
        # Candidates are the keys in 'invariants', plus 'assumptions' and 'build_dag'
        ...
        for err in errors:
            ...
            # Map error locations to lockable sections
            if parts[0] == 'invariants' and len(parts) > 1:
                tainted.add(parts[1])
            elif parts[0] == 'weak_format':
                # Weak format errors reference a specific field
                # We need to find which invariant section contains this field
                # This is hard without back-tracing, so we taint ALL invariants to be safe
                # (Strict, Hostile behavior)
                # But to be precise, if we passed 'reference', we could try.
                # For now, taint all invariants if weak format fails is safer.
                return [] 
            elif parts[0] in candidates:
                tainted.add(parts[0])
            elif parts[0] == 'schema_version' or parts[0] == 'structure':
                # Structural failure implies we cannot trust the tree -> LOCK NOTHING
                return []
            else:
                # Unknown error location -> LOCK NOTHING
                return []
